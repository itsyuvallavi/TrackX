...
Status: Production foundations are live. Vercel serves the authenticated Next.js app and same-origin API, Supabase stores production data and audit events, and Cloudflare receives Telegram webhooks. The current release hardens centralized observability, acknowledges Apple Wallet imports before background parsing, and adds per-user merchant-category memory. Supabase remains the durable audit source, Better Stack is the sanitized central operational view, and Vercel/Cloudflare runtime logs are failure-only fallbacks.
